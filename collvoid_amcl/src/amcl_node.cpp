        ...
        first_reconfigure_call_(true)
{
  ...
  particlecloud_weighted_pub_ = nh_.advertise<collvoid_msgs::PoseArrayWeighted>("particlecloud_weighted", 1, true);
...
AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
  ...
  if(resampled || force_publication) {
    ...
    if (max_weight > 0.0 && false) {
      ...
      cloud_msg2.header.stamp = laser_scan->header.stamp;
      cloud_msg2.header.frame_id = global_frame_id_;
      cloud_msg2.poses.resize(hyps[max_weight_hyp].count);
      cloud_msg2.weights.resize(hyps[max_weight_hyp].count);
      int count = 0;
