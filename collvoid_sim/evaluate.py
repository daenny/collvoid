...
def bounding_box(pos):
    if pos[0] < -4.1 or pos[0] > -0.1 or pos[1] < 0.0 or pos[1] > 5.0 :
...
if __name__== '__main__':
    ...
    for topic, msg, t in bag.read_messages():
        ...
        if not robot_name in runs[run]:
            ...
            if dt > 0:
                ...
                robot['temp_distance'] = 0
                robot['temp_last_time'] = msg.ground_truth.header.stamp
    ...
    for run in range(len(runs)):
        ...
        for robot_name in runs[run]:
            ...
            print "%s\ttime:\t%f\tdist:\t%f\tavg_loc_error:\t %f\tcost_jerk_lin: %f"%(robot_name, time, distance, avg_loc_error, cost_jerk_lin)
    ...
    loc_err_std = numpy.mean(loc_err_array)
    ...
    print "-" * 80
    print "#robots: %d\nlocalization: %s\n#runs: %d\n#runs w/ collision: %d\n#runs deadlock: %d\n#runs out of box: %d\n#runs not started: %d\n#runs small jerk: %d\navg max time: %f\t%f\navg distance: %f\t%f\navg-jerk-lin-cost: %f\t%f\navg-jerk-ang-cost: %f\t%f\nloc-err: %f\t%f"%(num_robots, str(localization), run_count, collision_count, deadlock_count, out_of_box_count, not_started_count, small_jerk_count, time_max_avg, time_max_std, distance_avg, distance_std, jerk_lin_avg, jerk_lin_std, jerk_ang_avg, jerk_ang_std, loc_err_avg, loc_err_std)
