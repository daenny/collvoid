...
def create_world_file(argv):
    ...
    try:
        opts, args= getopt.getopt(argv, "hn:s:Rolrxf:", ["help","numRobots=","circleSize=","radiusScaling","holo","truePosition","real","experiments","bagFileName"])
    ...
    worldFileTemp.close()
    worldFileNew.close()
    create_yaml_file(circleSize,numRobots,holo,simulation,truePos,centerX,centerY,scaleRadius)
    create_launch_file(numRobots,holo,runExperiments,bagFileName)
    
    
def create_yaml_file(circleSize, numRobots,holo,simulation,truePos,centerX,centerY,scaleRadius):
    direct = commands.getoutput('rospack find collvoid_sim')
...
def create_launch_file(numRobots,holo,runExperiments, bagFilename):
    direct = commands.getoutput('rospack find collvoid_sim')
    ...
    if (runExperiments):
        launchWrite.write('<node pkg="controller" type="controllerExp.py" name="controller" output="screen"/>\n')
    else:
...
#        launchWrite.write('<node pkg="path_listener" type="listener.py" name="listener" ns="robot_{0}" output="screen"/>\n'.format(x))
    ...
    launchWrite.write('<node pkg="rosbag" type="record" name="rosbag" args="record {0} -O $(find collvoid_sim)/{1}" output="screen"/>\n'.format(s,bagFilename))
